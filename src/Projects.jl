...
get_metadata(project::DSSProject=get_current_project()) = request("GET", "projects/$(project.key)/metadata")
set_metadata(project::DSSProject, body::AbstractDict) = set_metadata(body, project)
set_metadata(body::AbstractDict, project::DSSProject=get_current_project()) = request("PUT", "projects/$(project.key)/metadata", body)

get_settings(project::DSSProject=get_current_project()) = request("GET", "projects/$(project.key)/settings")
set_settings(project::DSSProject, body::AbstractDict) = set_settings(body, project)
set_settings(body::AbstractDict, project::DSSProject=get_current_project()) = request("PUT", "projects/$(project.key)/settings", body)

get_permissions(project::DSSProject=get_current_project()) = request("GET", "projects/$(project.key)/permissions")
set_permissions(project::DSSProject, body::AbstractDict) = set_permissions(body, project)
set_permissions(body::AbstractDict, project::DSSProject=get_current_project()) = request("PUT", "projects/$(project.key)/permissions", body)

get_variables(project::DSSProject=get_current_project()) = request("GET", "projects/$(project.key)/variables")
set_variables(project::DSSProject, body::AbstractDict) = set_variables(body, project)
set_variables(body::AbstractDict, project::DSSProject=get_current_project()) = request("PUT", "projects/$(project.key)/variables", body)

get_tags(project::DSSProject=get_current_project()) = request("GET", "projects/$(project.key)/tags")
set_tags(project::DSSProject, body::AbstractDict) = set_tags(body, project)
set_tags(body::AbstractDict, project::DSSProject=get_current_project()) = request("PUT", "projects/$(project.key)/tags", body)

export_project(project::DSSProject; options...) = request("GET", "projects/$(project.key)/export"; params=Dict(options), stream=true)
